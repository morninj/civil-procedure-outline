...
\begin{enumerate}
    \item \textbf{Territorial jurisdiction}: jurisdiction over cases 
    arising in or involving people residing within a defined territory. 
    ...
    questions.
    \item \textbf{Venue}: is this the right court within a proper jurisdiction 
    in which to sue?
    \item  \textbf{\emph{Forum non conveniens}}: is there a jurisdiction that 
    is dramatically more appropriate?
    \item Parties must raise objections to jurisdiction at the beginning of a 
    suit.
    \item A judgment \textbf{void when rendered is void forever} and is 
    therefore vulnerable to \textbf{collateral attack}. \emph{Pennoyer}.
\end{enumerate}
...
\begin{enumerate}
    ...
    personal jurisdicion over someone outside of its physical territory.  
    \emph{Pennoyer}. 
    \item States can require implied consent to jurisdiction from out-of-state 
    drivers. \emph{Hess}.
    \item In personam jurisdiction is established if the defendant has 
    \textbf{minimum contacts} with the forum state. \emph{International Shoe}.
    \item General vs. specific jurisdiction:
    \begin{enumerate}
        ...
        \item A \textbf{stream of commerce} connection is insufficient to 
        establish general jurisdiction. \emph{Goodyear}.
    \end{enumerate}
    \item \textbf{Purposeful availment} means benefiting from the protection 
    of the forum state's laws. See \emph{World-Wide Volkswagen} below. Selling 
    products that end up in the forum state is insufficient (\emph{World-Wide 
    Volkswagen}), but establishing a franchise relationship is enough 
    (\emph{Burger King}).
    \item \textbf{Purposeful direction} establishes general jurisdiction, but 
    it does not exist if the seller did not explicitly intend to sell products 
    to the specific forum state.\emph{J. McIntyre}.
    \item \textbf{Long-arm statutes} authorize courts to exercise jurisdiction 
    over people beyond their borders. See more below and \emph{Bensusan}.
    \item \textbf{Rule 4(k)}---federal personal jurisdiction analysis:
    \begin{enumerate}
        \item If the claim arises under \emph{state} law, apply the state 
        long-arm statute.
        \item Is there a federal long-arm statute? If so, apply it and 
        check whether the defendant has minimum contacts with the US. (If 
        there is no personal jurisdiction in any state, you can sue in any 
        district court.) If not, apply the state long-arm statute. 
    \end{enumerate}
    \item Quasi in rem jurisdiction is also based on the minimum contacts 
    test, not the territorial power framework.\emph{Shaffer}.
    \item \textbf{Physical presence} establishes personal jurisdiction. 
    \emph{Burnham.} Property does not. \emph{Shaffer.}
    \item Due process requires \textbf{notice and opportunity to be heard}. 
    \emph{Mullane.} The government must take affirmative steps to ensure 
    adequate notice (\emph{Flowers}).
    \item \textbf{Forum selection clauses} in contracts of adhesion are 
    enforceable means of establishing personal jurisdiction. \emph{Carnival 
    Cruise Lines.}
\end{enumerate}
...
\subsubsection{Product Distribution and Purposeful Direction: \emph{J. 
...
\subsubsection{Online Activity and Long-Arm Statutes: \emph{Bensusan Rest. 
Corp v. King}}
...
\subsubsection{``Notice Reasonably Calculated'' \emph{Mullane v. Central 
Hanover Bank \& Trust}}
...
\begin{enumerate}
    ...
    \item The two types of subject matter jurisdiction we deal with are 
    \textbf{federal question} and \textbf{diversity} (see each section below).
    ...
    between state and federal courts. Parties can raise subject matter 
    jurisdiction issues at any point, and courts can raise it \emph{sua 
    sponte} (as the Supreme Court did in \emph{Mottley}).
...
\begin{enumerate}
    ...
    \item \textbf{28 U.S.C \S\ 1331 establishes federal question 
    jurisdiction}: ``The district courts shall have original jurisdiction of 
    all civil actions arising under the Constitution, laws, or treaties of the 
    United States.''
    \item \textbf{States have concurrent jurisdiction over cases based on 
    federal law} unless Congress has provided for exclusive 
    jurisdiction.\footnote{Casebook 
    p. 373.}
    \item \textbf{Federal courts have concurrent jurisdiction over state law 
    cases} as long as there is subject matter jurisdiction. Diversity is 
    typically the basis for federal subject matter jurisdiction, though 
    federal questions are also often involved.\footnote{Casebook p. 373--374.}
    ...
    \emph{Osborn v. Bank of U.S.}, 22 U.S. 738, 823 (1824).}
    \item Federal cases must be based on \textbf{well pleaded complaints}, 
    i.e., an anticipated federal defense is not sufficient to establish 
    federal question jurisdiction. \emph{Mottley}.
    \item Even if there is no federal cause of action, federal courts can 
    choose to hear state law causes if they \textbf{implicate significant 
    federal issues}. \emph{Grable}.
    \item Complaints must satisfy the \textbf{well-pleaded complaint rule}. 
    \emph{Mottley}.
    \item Courts can hear state law issues if they \textbf{implicate 
    significant federal issues}. \emph{Grable}.
\end{enumerate}

\paragraph{Well Pleaded Complaint: \emph{Louisville \& Nashville R.R. Co. v.  
Mottley}}
~\\\\
...
\begin{enumerate}
    ...
    \item \textbf{28 U.S.C. \S\ 1332 authorizes diversity jurisdiction}. 
    Federal courts have original jurisdiction over all cases where the amount 
    in controversy is greater than \$75,000 and the parties meet the diversity 
    requirements.
    \item \textbf{Complete diversity} requires that none of the plaintiffs can 
    be from the same state as any of the defendants. \emph{Mas}.
    ...
    \item Plaintiffs' claims must meet the \textbf{amount in controversy} 
    requirements (currently, \$75,000). A plaintiff can aggregate multiple 
    claims against a defendant to meet the amount in controversy 
    requirement---however, a plaintiff cannot aggregate multiple claims 
    against multiple defendants.
    \item Corporations are citizens of their state of \textbf{incorporation} 
    and the state where their \textbf{``nerve center''} is located. 
    \emph{Hertz}.
\end{enumerate}
...
\begin{enumerate}
    \item Supplemental jurisdiction \textbf{allows a claim without jurisdiction to 
    join a claim with valid jurisdiction}.
    \item \textbf{28 U.S.C. \S\ 1367}, passed in 1990, codified the federal 
    common law rules of pendent and ancillary jurisdiction into the single 
    concept of supplemental jurisdiction.
    \item The court has \textbf{discretion not to exercise supplemental 
    jurisdiction} under 28 U.S.C. \S\ 1367(c).
    \item Evaluating supplemental jurisdiction cases:
    \begin{enumerate}
        \item Is there a claim with valid original jurisdiction?
        \item Do supplemental claims form part of the \textbf{same case or 
        controversy?} 28 U.S.C. \S\ 1367(a).
        \item Are the supplemental claims \textbf{within the \S\ 1367(b) 
        rules?} A supplemental claim against a third-party defendant is not 
        allowed if (1) the original basis for jurisdiction is diversity and 
        (2) the supplemental claim would destroy diversity. \S\ 1367(b) exists 
        to prevent plaintiffs from bringing suit against diverse defendants 
        and waiting for them to implead a non-diverse defendant who is the 
        real target of the action. See \emph{Owen}.
    \end{enumerate}
    \item \emph{Gibbs}'s ``common nucleus of operative fact'' requirement is 
    preserved in \S\ 1367(a) as the ``same case or controversy.''
    ...
    the claim would destroy diversity jurisdiction.
\end{enumerate}

\paragraph{Common Nucleus of Operative Fact: \emph{United Mine Workers of Am. 
v. Gibbs}} 

Federal courts can exercise supplemental [pendent] jurisdiction if the claims 
...
third-party defendants if the claim would destroy diversity jurisdiction.
...
\paragraph{Removal and Subject Matter Jurisdiction: \emph{Caterpillar Inc. v.  
Williams}}
~\\\\
...
\begin{enumerate}
    \item Broadly, venue tells you which district you can sue in.
    \item Venue refers ``to the \textbf{geographic specification of the proper 
    court or courts} for the litigation of a civil action that is within the 
    subject matter jurisdiction of the district courts in general.'' 28 U.S.C. 
    \S\ 1390(a). I.e., which court \emph{within the proper system} can a 
    plaintiff bring a case?
    \item \textbf{The primary purpose of venue is to protect the 
    defendant.}\footnote{Casebook p. 449.}
    \item 28 U.S.C. \S\ 1391, \textbf{``Venue generally''}: determines the 
    federal district where a defendant can be sued. \textbf{Venue cannot 
    create personal jurisdiction where none otherwise exists.} It can only 
    \emph{narrow}, not expand.
    \item 28 U.S.C. \S\ 1404, \textbf{``Change of venue''}: determines when 
    and how a case can be transferred between federal districts---``for 
    convenience~.~.~.~in the interest of justice~.~.~.~'' Federal courts 
    cannot transfer to state courts.
    \item 28 U.S.C. \S\ 1406: \textbf{``Cure of waiver of defects''}: how to 
    deal with improper venue.
    ...
    \item \textbf{Transfer of venue}: state $\rightarrow$ state or federal $\rightarrow$ 
    federal. Removal: state $\rightarrow$ federal.
    \item \textbf{\emph{Forum non conveniens}} asks whether the there's a more 
    convenient forum where the case should be adjudicated. \emph{Piper}.
    \item Successful \emph{forum non conveniens} motions result in dismissal.
    \item Differences in substantive law are insufficient to dismiss under 
    \emph{forum non conveniens} unless the law in the target forum is 
    egregiously bad. \emph{Piper}.
\end{enumerate}
